...
// ============================================================================
// Bank Transfer (for donations >= ₹5,00,000)
// ============================================================================

import type { BankTransferPayload } from "@/lib/schemas/donation";
...
export const generateReferenceCode = (): string => {
  ...
  return `MUL-BT-${dateStr}-${randomStr}`;
};

/**
 * Submit a bank transfer donation (for amounts >= ₹5,00,000)
 * This bypasses Razorpay and creates a pending verification record
 */
export const submitBankTransfer = async (data: BankTransferPayload) => {
  try {
    const response = await publicGateway.post(donationRoutes.bankTransfer, {
      amount: data.amount,
      name: data.name,
      donation_name: data.donationName,
      company: data.isOrganisation ? data.organisationName : undefined,
    ...
    toast.error(getApiErrorMessage(error, "Error submitting bank transfer. Please try again."));
    throw error;
  }
};
